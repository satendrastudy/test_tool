...
# Write data to specific cells
    ...
    #sheet["K2"] = sum_Outward['Description'][0]
    #sheet["L2"] = sum_Outward['Amount'][0]
...
for i in range(1, 32):  # Loop from 1 to 31
    ...
    for l in range(1, 2):
        ...
        patterns = [
            f'ACH*.txt'
            #f'*{k}052025-0000001-R3C5-{m}-RES.txt',
            #f'*{k}052025-0000002-R3FC-{m}-RES.txt',
           # f'*{k}052025-0000001-R3FC-{m}-RES.txt',
           # f'*{k}052025-0000003-R3FC-{m}-RES.txt',
           # f'*{k}052025-100001-R3FC-{m}-RES.txt'
        ]
...
for i in range(1, 32):  # Loop from 1 to 31
    ...
    try:
        ...
        #sheet.merge_cells(f'H{aa}:I{aa}')
        #sheet[f'H{aa}'] ='Nach Pool File is missing'
        #sheet[f'H{aa}'].font = openpyxl.styles.Font(bold=True, size=12,color="e61919")
        #wb.save(df_natch_recon)
...
# %%
#D_df1=df[~df['Tran Ref. No'].str.match(r'^\d{1,3}/', na=False)].drop_duplicates(subset=['Particulars'], keep='first')

# %%
df23 = df[~(df['Particulars_1'] == 'ACH Debits')]
#df = df[df['Particulars'].str.contains('ACH Debits Return', na=False)]
df23
...
# Filter rows where 'Particulars' contains 'UPI SETTLEMENT ON'
#read_file3 = read_file3[read_file3['Particulars'].str.contains('ACH Debit Presentation', case=False, na=False)]
read_file3
...
# %%
#debit_sum_by_date_rbi = debit_sum_by_date_rbi.rename(columns={"Debits": "Debits_RBI"})
